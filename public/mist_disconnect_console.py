...
def build_verdict(
    ...
    return {"primaryCause": primary, "notes": notes, "correlations": cors}
...
def self_test() -> None:
    ...
    assert "score" not in v and "label" not in v, v
    empty = build_verdict(None, [], [])
    assert "score" not in empty and "label" not in empty, empty
    assert empty["primaryCause"].startswith("No dominant failure signature"), empty
    ...
    assert "score" not in demo["verdict"] and "label" not in demo["verdict"], demo["verdict"].keys()
    assert demo["verdict"]["primaryCause"], demo["verdict"]
